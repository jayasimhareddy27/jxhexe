...
export const Scrapedjobheader = ({ jobData, dispatch, isAnyLoading, flashDeepData, isLazyLoading }) => {
  const handleChange = (field, value) => {
        dispatch(updateJobData({ [field]: value }));
    };
    return(<>
        {/* COMPANY & POSITION */}
        <section className="p-3 rounded-2xl border-2 border-slate-100 bg-slate-50/30 flex justify-between items-start gap-4">
          <div className="flex-1 min-w-0">
            <input 
                className="w-full text-lg font-black outline-none bg-transparent truncate" 
                placeholder="Company" 
                value={jobData.companyName || ""} 
                onChange={e => handleChange('companyName', e.target.value)} 
            />
            <input 
                className="w-full text-xs font-bold text-indigo-600 outline-none uppercase tracking-tight bg-transparent truncate" 
        ...
        <div className={`grid grid-cols-2 gap-2 transition-all duration-500 ${flashDeepData ? 'scale-[1.02]' : 'scale-100'}`}>
            <div className={`p-2 rounded-xl border transition-all ${flashDeepData ? 'border-indigo-400 bg-indigo-50 shadow-sm' : 'bg-slate-50 border-slate-100'}`}>
                ...
                <select 
                    className="text-[10px] font-bold outline-none w-full bg-transparent text-slate-700 cursor-pointer" 
        ...
        <section className="space-y-1.5">
          ...
          <div className="grid grid-cols-4 gap-2">
            <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
              ...
              <input 
                className="text-[11px] font-bold outline-none w-full bg-transparent" 
            ...
            <div className="bg-slate-50 p-2 rounded-xl border border-slate-100 flex flex-col justify-center">
               ...
               <select 
                className="text-[10px] font-bold outline-none w-full bg-transparent" 
        ...
        <section className="space-y-1.5 relative">
          ...
          <textarea 
              ...
              onChange={e => handleChange('rawDescription', e.target.value)} 
          />
        </section>
        ...
        <div className="grid grid-cols-2 gap-2">
            <div className="bg-slate-50 p-2.5 rounded-xl flex items-center gap-2 border border-slate-100">
                ...
                <input 
                    className="text-[10px] font-bold outline-none w-full bg-transparent truncate" 
